...
- メモの所有者は、`username` で指定したユーザーにメモを共有できる
- 1つのメモを複数のユーザーに共有できる
- 1回の共有操作で共有できるメモは1つだけとする
- 共有操作では対象の `memo_id` と共有先の `username` を受け取る
- 共有先の `user_id` はクライアントから受け取らず、バックエンドが `username` から取得する
- 共有されたユーザーに公開する情報は、所有者の `user_id`・`username` とメモの内容だけとする
- 共有されたユーザーはメモを閲覧できる
...
- 画面上のニックネームと、共有相手を検索する公開IDの両方に使用する
- 日本語を使用できる
- 前後の空白を除去して保存する
...
共有関係には `username` ではなくUUIDの `user_id` を保存する。
...
- 1回の共有操作では、1つの `memo_id` と1人分の `username` を受け取る
- 共有相手は `Profiles.username` で検索する
...
| 対象 | 用途 |
| --- | --- |
| `Profiles.username` | 共有相手の検索、一意性の保証 |
